...
export function TubeLightNavBar({ items, className, firstName }: NavBarProps) {
  ...
  return (
    <div className={cn("fixed z-[50] top-0 left-1/2 -translate-x-1/2 mb-6 md:pt-8 pt-4", className)}>
      <div className="flex items-center gap-3 border border-myPalette4 backdrop-blur-sm py-2.5 px-3.5 rounded-full shadow-lg">
        <Link to={"/"} className="flex items-center gap-2 mr-14 cursor-none">
          <img className="w-12 aspect-square object-cover rounded-full" src={profilePhoto} alt={`${fullName} Profile Photo`} />
          <p className="font-myNameFont text-xl text-myPalette9">{firstName}</p>
        </Link>
        {items.map((item) => {
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive && "bg-myPalette9 hover:bg-myPalette9 text-myPalette1 hover:text-myPalette1",
              )}
            >
